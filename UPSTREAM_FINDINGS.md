...
| 30 | [`compile --target python` never imports referenced types from other modules - annotations still do not resolve cross-module](#30-compile---target-python-never-imports-referenced-types-from-other-modules---annotations-still-do-not-resolve-cross-module) | Crash (broken generated code) | A | Semantic-typed half fixed (now emits bare primitives); value-typed half still open (missing import), same-domain included — re-verified v1.9.4 |
