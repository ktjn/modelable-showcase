...
| 27 | [`compile --target sql-postgres` emits `FOREIGN KEY (...)` referencing the model name, not the bound table name - the full graph cannot be applied](#27-compile---target-sql-postgres-emits-foreign-key--referencing-the-model-name-not-the-bound-table-name---the-full-graph-cannot-be-applied) | Crash (broken generated code) | A | Fixed in the current upstream fix branch |
...
**Status:** Fixed in the current upstream fix branch. The SQL emitter now
...
**Status:** Fixed in the current upstream fix branch. `ref<>` fields now use
...
**Status:** Fixed in the current upstream fix branch. Rust field identifiers
...
**Status:** Fixed in the current upstream fix branch. TypeScript projection
