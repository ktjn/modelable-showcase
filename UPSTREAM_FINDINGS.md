...
| 30 | [`compile --target python` never imports referenced types from other modules - annotations still do not resolve cross-module](#30-compile---target-python-never-imports-referenced-types-from-other-modules---annotations-still-do-not-resolve-cross-module) | Crash (broken generated code) | A | Fixed in v1.9.5 (via [ktjn/modelable#418](https://github.com/ktjn/modelable/pull/418)) |
...
**Status:** Fixed in **v1.9.5** (via [ktjn/modelable#417](https://github.com/ktjn/modelable/pull/417), merged) - both defects originally described here are genuinely resolved. The residual defect that surfaced during PR review (a `BackboneElement`-typing gap this finding's own reproduction never reached, since it was masked by these two more fundamental defects) is now separately fixed too - see [## 45](#45-compile---target-fhir-profile-emits-extensionurl-elements-with-no-explicit-type-so-snapshot-generation-still-fails-the-official-hl7-fhir-validator-even-after-417). `tests/integration/test_generated_artifacts.py::test_fhir_profiles_pass_the_hl7_validator` now asserts success against the pinned 1.9.5 release.
...
**Status:** Fixed in **v1.9.5** (via [ktjn/modelable#417](https://github.com/ktjn/modelable/pull/417), merged). `avro` is now a real implemented target on the pinned release (`modelable capabilities --format json` reports 21 implemented targets on 1.9.5, `avro` included) - `compile --target avro` against this showcase's real `model/` (including `billing.mdl`'s defaulted `tax: decimal(10, 2) = 0` fields, this finding's own reproduction) compiles cleanly with no traceback, confirmed by `make generate`/`make determinism` and `tests/integration/test_avro_codegen.py`. `tests/conformance/capability-coverage.yaml` gained a `target:avro` entry accordingly. A distinct, separate gap surfaced during that new coverage - avro never resolves a multi-field named-type reference at all (broader than the cross-domain-only defect other targets had before their own fixes, #28/#29/#31 etc.) - see [## 47](#47-compile---target-avro-never-resolves-any-multi-field-named-type-reference-value-or-entity-same-domain-or-cross-domain---it-degrades-to-a-lossy-string-fallback-instead).
...
**Showcase workaround:** None needed, and none was ever adopted - `avro` was not an implemented target on the pinned `1.9.4` release this showcase depended on when this was found, so no `.mdl` file, generation step, or test in this repository was affected at the time. Logged purely as a canary finding (`UPSTREAM_POLICY.md` §6 Case A) exactly so it would not need rediscovering from scratch once a re-pin adopted a release including the `avro` target - which is what happened with the 1.9.5 re-pin above.
...
**Status:** Fixed in **v1.9.5**, via this showcase's own upstream contribution [ktjn/modelable#419](https://github.com/ktjn/modelable/pull/419) (merged). #418's own two revisions (below) never fully closed this - the second revision's fix for the extension files' `value[x]` shifted the defect into the top-level profile's `Patient.contact` element instead of eliminating it. #419 was the real root-cause fix: reordering the differential to match every base resource's own structural element order (extension before resource-specific fields), correctly typing composite direct fields as `BackboneElement` (scoped so `Extension.value[x]`, which forbids it, is never affected), and filling in `base`/`definition`/`type` on elements that were missing them entirely (`Patient.extension`'s own slicing element, every extension slice/value element). Verified against the actual 1.9.5 release (not just the PR branch): `clinical.PatientFhirView.v1.fhir.json` together with its extensions and the shared `pii`/`classification` extensions now validates with **0 errors** (`tests/integration/test_generated_artifacts.py::test_fhir_profiles_pass_the_hl7_validator`). A distinct, separate defect surfaced during this verification - see [## 46](#46-compile---target-fhir-profile-uses-a-generic-backboneelement-fallback-for-composite-fields-that-map-to-a-more-specific-base-fhir-type-eg-observationcode-needing-codeableconcept).

**#418's own history** (superseded by #419 above, kept for context): still open after two revisions of [ktjn/modelable#418](https://github.com/ktjn/modelable/pull/418). Verified by installing each exact commit and re-running this finding's own reproduction against the real HL7 FHIR Validator (`6.10.2`):
...
modelable compile ./model --target fhir-profile --out /tmp/fhir-out --registry /tmp/reg.db --registry-ids /tmp/ids.lock
java -jar tools/validator_cli.jar \
  /tmp/fhir-out/clinical.ObservationFhirView.v1.fhir.json \
  /tmp/fhir-out/pii.fhir.json \
  /tmp/fhir-out/classification.fhir.json \
  -version 4.0.1
```
...
Error @ StructureDefinition: Error generating Snapshot: StructureDefinition
http://modelable.io/fhir/StructureDefinition/clinical.ObservationFhirView.v1
at Observation.code: invalid constrained type BackboneElement from
CodeableConcept in http://hl7.org/fhir/StructureDefinition/Observation
(this usually arises from a problem in the differential)
```

**Root cause (read from source, not guessed - `emitters/fhir.py` on the pinned 1.9.5 release):** `_fhir_type`'s `allow_backbone=True` path (used for genuine base-resource direct fields, added by #419) unconditionally returns `[{"code": "BackboneElement"}]` for any composite `NamedType`/`ObjectType` field once resolution reaches that fallback - there is no lookup against the real base FHIR resource's own declared type for that specific field path. `Patient.contact`'s real base type happens to be `BackboneElement`, so #419's fix is correct there, but `Observation.code`'s real base type is `CodeableConcept` (confirmed via the same defect class also reproducing for `Patient.address`, which needs `Address`, and `Encounter.reasonCode`, which needs `CodeableConcept` - not independently isolated into their own findings since they're the same root cause, just different affected base resources/fields).
...
**Status:** Open. Surfaced immediately on adopting `avro` as a real target with the 1.9.5 re-pin (finding #44 fixed the crash that previously made this target unusable at all).

**Discovered:** Compiling this showcase's real `model/` to `avro` for the first time (`tests/conformance/capability-coverage.yaml`'s new `target:avro` entry) and reading the emitted `.avsc` files' `WARN [EMIT002]` diagnostics.

**Reproduction:**

```bash
modelable compile ./model --target avro --out /tmp/avro-out --registry /tmp/reg.db --registry-ids /tmp/ids.lock
```

**Observed:**

```text
WARN [EMIT002] Type 'unresolved Avro named type Address' cannot be represented without loss
```

`generated/avro/patient/PatientEvent.v2.avsc`'s `address` field (a same-domain reference to the multi-field value type `PatientAddressV0`) renders as `["null", "string"]` instead of a resolved named Avro record; `generated/avro/patient/Patient.v2.avsc`'s required `contact` field (same-domain reference to `PatientContactDetailsV0`) renders as bare `"string"`. Neither is cross-domain, ruling out that framing - this reproduces for any multi-field named-type reference, same-domain included.

**Root cause (read from source, not guessed - `emitters/avro.py`'s `_type_schema`, installed release `1.9.5`):**
...
The `NamedType` branch only ever attempts `resolve_semantic_type_ref` - which resolves *semantic* types only. A `NamedType` referencing a *value* type (like `PatientAddressV0`/`PatientContactDetailsV0`) or an entity is never a semantic type, so this always raises, and the emitter falls through to the generic lossy `"string"` fallback unconditionally - there is no equivalent of `_fhir_type`'s (or the other emitters') `resolve_model_ref`-based general named-type resolution here at all.
