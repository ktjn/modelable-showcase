...
| 45 | [`compile --target fhir-profile` emits `Extension.url` elements with no explicit type, so snapshot generation still fails the official HL7 FHIR Validator even after #417](#45-compile---target-fhir-profile-emits-extensionurl-elements-with-no-explicit-type-so-snapshot-generation-still-fails-the-official-hl7-fhir-validator-even-after-417) | Invalid generated output | A | Partially fixed in [PR #418](https://github.com/ktjn/modelable/pull/418) (draft) — see [## 45](#45-compile---target-fhir-profile-emits-extensionurl-elements-with-no-explicit-type-so-snapshot-generation-still-fails-the-official-hl7-fhir-validator-even-after-417) for residual `BackboneElement` defect |
...
**Status:** Partially fixed by [ktjn/modelable#418](https://github.com/ktjn/modelable/pull/418) (draft). Verified by installing that exact branch (`git+https://github.com/ktjn/modelable@agent/fix-final-upstream-gaps#subdirectory=cli`) and re-running this finding's own reproduction against the real HL7 FHIR Validator (`6.10.2`): PR #418 adds `"type": [{"code": "uri"}]` to `Extension.url` in both `_emit_extension_sd` and `_emit_annotation_extension_sd`, exactly as this finding's "Expected" section asked. `pii.fhir.json` and `classification.fhir.json` (the shared annotation-marker extensions, whose value is a simple primitive) now validate with **0 errors**. But the per-field extensions for value-typed/named-type fields (`clinical.PatientFhirView.v1.ext.patientId`, `.ext.legalName`, `.ext.dateOfBirth`) still fail validation with the exact same `sdf-3` and `Extension.value[x]: invalid constrained type BackboneElement` errors quoted below - PR #418's diff never touches `_fhir_type()`, which is the actual source of those two remaining errors (see updated root cause below). `clinical.PatientFhirView.v1.fhir.json` itself is byte-for-byte identical in failure count/content before and after #418 (5 errors, 1 warning, 1 note), because its own snapshot generation still cascades from the still-broken extensions. The PR description's claim to close "the two remaining open upstream findings" is accurate for #30 (see [## 30](#30-compile---target-python-never-imports-referenced-types-from-other-modules---annotations-still-do-not-resolve-cross-module)) but overstated for this finding - #45 is not fully closed.

**Original discovery** (superseded observations below by the #418 re-check, kept for history):
...
**Root cause, url-type half (fixed by #418):** `emitters/fhir.py` on PR #417's branch had `_emit_extension_sd`'s `elements` list set `"id": "Extension.url"`, `"path": "Extension.url"`, `"min": 1`, `"max": "1"`, `"fixedUri": ext_url` - no `"type"` key, and `_emit_annotation_extension_sd` had the identical gap. PR #418's diff is exactly two one-line additions of `"type": [{"code": "uri"}]` to those two `Extension.url` elements - confirmed by reading the PR's diff directly, not just its description.

**Root cause, remaining half (still open after #418 - read from `emitters/fhir.py` on PR #418's branch):** `_fhir_type(field_type, source_field=...)` (module-level helper, ~line 434) unconditionally maps `isinstance(field_type, (NamedType, ObjectType))` to `[{"code": "BackboneElement"}]`. `clinical.PatientFhirView.v1`'s `patientId`/`legalName`/`dateOfBirth` fields resolve to a `NamedType`/value-type, so `_extension_sd_value_element()` calls `_fhir_type()` and gets back `BackboneElement` for `Extension.value[x]` - which FHIR's own base `Extension.value[x]` element does not permit as a constrained type (its allowed type list is primitives/complex-datatypes only, never `BackboneElement`), so the validator's snapshot generator rejects it, which is exactly the `invalid constrained type BackboneElement ... in http://hl7.org/fhir/StructureDefinition/Extension` error. The `sdf-3` error on the same files is a direct cascade of this - snapshot generation aborts before every element gets a formal definition/cardinality. `clinical.PatientFhirView.v1.fhir.json`'s own unchanged failures are a further cascade: its differential references `Patient.contact`, `Patient.extension` against extensions whose snapshots never successfully generate.

**Expected:** `_fhir_type` needs a real mapping for `NamedType`/`ObjectType` field types used as an extension's own `value[x]` (as opposed to a sub-extension, which already has its own dedicated slicing path in `_extension_sd_sub_extension_elements`) - e.g. resolving to the named type's own underlying primitive/complex FHIR type, not the always-invalid `BackboneElement` fallback.
