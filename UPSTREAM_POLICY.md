...
These OpenAPI-specific requirements are folded into the single merged Definition of Done in `SPEC.md` §25 as items 20-26, so that document — not this one — is the one canonical completion checklist. This section stays here only as OpenAPI-specific process context; if the two ever disagree, `SPEC.md` §25 governs and this section should be corrected to match, not the reverse.

## 12. Next known upstream priority (informational, not yet actionable)

`ktjn/modelable`'s own `ROADMAP.md` lists Avro export/import hardening as its second P0 priority, immediately after OpenAPI. Avro is not currently a `capabilities --format json` target at all (it is deferred grammar-only vocabulary, see `IMPLEMENTATION_PLAN.md` Task 1.3), so no showcase task depends on it today and none should be added speculatively.

This section exists only so a future implementation agent recognizes the pattern early: if Avro (or any other ROADMAP P0/P1 item) becomes a real `compile --target` while this showcase is mid-implementation, treat it exactly like OpenAPI was treated here — verify against upstream `main`, decide whether the showcase has a natural product use for it, and apply the same §1 gap-handling sequence rather than bolting on an ad hoc side quest. Do not preemptively build Avro fixtures, tasks, or infrastructure before that happens.
