...
`uv run scripts/setup-full-database.py`) with a handful of synthetic
...
def wait_until_ready(base_url: str, timeout_s: float = 60.0) -> None:
    ...
    while time.monotonic() < deadline:
        try:
            ...
            pass
        time.sleep(1)
    print(f"seed-demo-data.py: {base_url}/ready did not become ready within {timeout_s}s", file=sys.stderr)
    sys.exit(1)
...
def seed_patient(base_url: str, spec: dict, index: int) -> str:
    ...
    if status != 201:
        raise RuntimeError(f"create patient {spec['legalName']!r} failed ({status}): {resp}")
    print(f"  patient      {spec['legalName']:<20} {patient_id}")
    return patient_id


def seed_appointment(base_url: str, patient_id: str, practitioner_id: str, scheduled_date: date, status: str) -> str:
    ...
    if resp_status != 201:
        raise RuntimeError(f"create appointment for {patient_id} failed ({resp_status}): {resp}")
    return appointment_id


def seed_encounter_with_observations(
    base_url: str, patient_id: str, practitioner_id: str, appointment_id: str, started_at: datetime
) -> str:
    ...
    body = {
        ...
        "endedAt": (started_at + timedelta(minutes=20)).isoformat().replace("+00:00", "Z"),
    ...
    if status != 201:
        raise RuntimeError(f"create encounter for {patient_id} failed ({status}): {resp}")
    ...
    observation = {
        ...
        "recordedAt": (started_at + timedelta(minutes=5)).isoformat().replace("+00:00", "Z"),
    }
    status, resp = request(base_url, "POST", f"/api/encounters/{encounter_id}/observations", observation)
    if status != 201:
        raise RuntimeError(f"create observation for encounter {encounter_id} failed ({status}): {resp}")
    return encounter_id


def seed_invoice(
    base_url: str, patient_id: str, encounter_id: str | None, issued_at: datetime, with_payment: bool
) -> str:
    ...
    if with_payment:
        payment = {
            ...
            "amount": "125.00",
            "method": "card",
            "receivedAt": (issued_at + timedelta(days=1)).isoformat().replace("+00:00", "Z"),
        }
        status, resp = request(base_url, "POST", f"/api/invoices/{invoice_id}/payments", payment)
        if status != 201:
            raise RuntimeError(f"record payment for invoice {invoice_id} failed ({status}): {resp}")
    return invoice_id


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument("--base-url", default="http://localhost:8080", help="apps/api base URL (default: %(default)s)")
    ...
    for index, spec in enumerate(PATIENTS):
        ...
        is_past = index < 4
        appointment_status = "completed" if is_past else "confirmed"
        scheduled_date = today - timedelta(days=7 - index) if is_past else today + timedelta(days=index)
        appointment_id = seed_appointment(base_url, patient_id, practitioner_id, scheduled_date, appointment_status)

        if appointment_status == "completed":
            started_at = datetime.combine(scheduled_date, datetime.min.time(), tzinfo=timezone.utc) + timedelta(hours=9)
            ...
            seed_invoice(base_url, patient_id, encounter_id, issued_at, with_payment=(index % 2 == 0))
