...
test.describe('GitHub Pages static build', () => {
  ...
  test('identifies and safely resets the browser-local sandbox', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Ada’s clinic journey is ready to inspect' })).toBeVisible()
    await page.getByRole('link', { name: 'Explore Ada’s record' }).click()
